...
OUTPUT_DIR = "config2"        # 模型和配置文件保存目录
BATCH_SIZE = 32                    # 批次大小
EPOCHS = 40                        # 训练轮次
LR = 6e-4                          # 初始学习率
Weight_decay=2e-4
...
def load_data():
    ...
    original_labels = df["label"].nunique()
    ...
    filtered_labels = df["label"].nunique()
    print(f"过滤后数据：{filtered_count}条样本，{filtered_labels}个类别（移除了样本数<10的类别）")
    if original_labels != filtered_labels:
        removed = original_labels - filtered_labels
        print(f"共移除{removed}个类别（每个类别样本数不足10）")
